...
async function upsertReviewsInDatabase(pool, reviews) {
  if (!pool || !Array.isArray(reviews) || reviews.length === 0) {
    return { written: 0, skippedInvalid: 0 };
  }
  ...
  return { written, skippedInvalid };
}

async function trySyncFileReviewsToDatabase(pool, fileReviews) {
  if (!pool || !Array.isArray(fileReviews) || fileReviews.length === 0) {
    return { written: 0, skippedInvalid: 0, attempted: 0 };
  }

  try {
    const result = await upsertReviewsInDatabase(pool, fileReviews);
    return {
      attempted: fileReviews.length,
    ...
    return {
      ...
      written: 0,
      skippedInvalid: 0,
      error: error instanceof Error ? error.message : "Ukendt fejl under sync.",
    };
  }
}

export async function getReviewStorageStatus() {
  ...
  const status = {
    ...
    dbCount: null,
    fileOnlyCount: fileReviews.length,
    invalidFileCount: 0,
    syncReady: Boolean(pool),
    syncError: null,
  };
  ...
  try {
    ...
    const dbKeys = new Set(dbReviews.map(getReviewKey));
    ...
    for (const review of fileReviews) {
      ...
      if (!dbKeys.has(getReviewKey(review))) {
        fileOnlyCount += 1;
      }
    }

    status.dbCount = dbReviews.length;
    status.fileOnlyCount = fileOnlyCount;
    status.invalidFileCount = invalidFileCount;
    return status;
  } catch (error) {
    status.syncError = error instanceof Error ? error.message : "Ukendt databasefejl";
    return status;
  }
}

export async function syncFileReviewsToDatabase() {
  const fileReviews = await readReviewsFile();
  const pool = getDbPool();
  ...
  if (!pool) {
    return {
      ok: false,
      reason: "DATABASE_URL er ikke sat.",
      attempted: fileReviews.length,
      written: 0,
      skippedInvalid: 0,
    };
  ...
  try {
    ...
    return {
      ok: false,
      reason: error instanceof Error ? error.message : "Ukendt fejl under sync.",
      attempted: fileReviews.length,
      written: 0,
      skippedInvalid: 0,
    };
