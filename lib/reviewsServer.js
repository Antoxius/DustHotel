...
export async function addReview(review) {
  ...
  try {
    ...
    console.error("Failed to save review to database. Falling back to file storage.", error);
    const reviews = await readReviewsFile();
    const next = [review, ...reviews];
    await writeReviewsFile(next);
    return review;
...
export async function upsertReviews(reviews) {
  ...
  try {
    ...
    console.error("Failed to upsert reviews to database. Falling back to file storage.", error);
    const existing = await readReviewsFile();
    const byId = new Map(existing.map((item) => [item.id, item]));
